...
- Вопросы: `toolId`, имя, тип, категория (id/name/icon/order), сцены для автоподключения модуля, `moduleId` для регистрации (выбор из модулей, у которых есть `<id>_module.ts`; иначе текстовый ввод).
- Артефакты:
